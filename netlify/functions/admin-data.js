...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (event.httpMethod === 'PATCH') {
      ...
      const sBase = process.env.SUPABASE_URL + '/rest/v1';
      ...
      if (TABLES[resource]) {
        ...
        const updRes = await fetch(`${sBase}/${TABLES[resource]}?id=eq.${rowId}`, {
          method: 'PATCH',
          headers: { ...sHdrs, 'Prefer': 'return=representation' },
          body: JSON.stringify({ data: merged, updated_at: new Date().toISOString() })
        });
        if (!updRes.ok) throw new Error(`Supabase update: ${updRes.status} ${await updRes.text()}`);
        return { statusCode: 200, headers: CORS, body: JSON.stringify(merged) };
      }

      // Config / non-table resources — read/modify/write
      const raw  = await store.get(resource);
      const data = raw ? JSON.parse(raw) : [];
      const arr  = Array.isArray(data) ? data : (data.items || []);
