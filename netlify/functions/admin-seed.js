...
// Seeds test data using Netlify Blobs REST API directly
...
exports.handler = async (event) => {
  ...
  if (!siteID || !token) {
    return { statusCode: 500, body: JSON.stringify({
      ...
      have: { siteID: !!siteID, token: !!token },
      hint: 'Add NETLIFY_SITE_ID and NETLIFY_TOKEN in Netlify > Site config > Environment variables'
    })};
  }
  ...
  const sponsors = [
    ...
    { id:'rr_sp003', createdAt:d(14), updatedAt:d(14), company:'Ybor City Tobacco Co.',          tier:'Bronze',   firstName:'Sandra', lastName:'Reyes',    email:'sreyes@yborcigars.com',       phone:'(813) 555-0202', amount:1000, status:'invoice', hole:'',      compSlots:1, notes:'Invoice sent 5/1.' },
  ...
  try {
    await blobSet('entries',  entries);
    await blobSet('sponsors', sponsors);
    await blobSet('comp',     comp);
    await blobSet('settings', settings);
    return { statusCode: 200, headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ success: true, entries: entries.length, sponsors: sponsors.length, comp: comp.length }) };
  } catch(err) {
